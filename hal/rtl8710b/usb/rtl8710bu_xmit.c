...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz, u8 bagg_pkt)
{
	...
	rtl8710b_update_txdesc(pxmitframe, (u8 *)ptxdesc);
...
static s32 rtw_dump_xframe(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		/* rtl8710b_update_txdesc(pxmitframe, mem_addr+PACKET_OFFSET_SZ); */
...
s32 rtl8710bu_xmitframe_complete(PADAPTER padapter, struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
...
s32 rtl8710bu_hal_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
static void rtl8710bu_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8710bu_hostap_mgnt_xmit_entry(PADAPTER padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
	...
	rtl8710b_cal_txdesc_chksum(ptxdesc);
	/* ----- end of fill tx desc ----- */
	...
	usb_fill_bulk_urb(urb, pdvobj->pusbdev, pipe,
		pxmit_skb->data, pxmit_skb->len, rtl8710bu_hostap_mgnt_xmit_cb, pxmit_skb);
