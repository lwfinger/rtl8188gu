...
void interrupt_handler_8188gtvu(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
	...
	if (pHalData->IntArray[0] & IMR_BCNDMAINT0_8188GTV)/*only for BCN_0*/
	...
	if (pHalData->IntArray[1]  & IMR_TXERR_8188GTV)
		RTW_INFO("===> %s Tx Error Flag Interrupt Status\n", __func__);
	if (pHalData->IntArray[1]  & IMR_RXERR_8188GTV)
		RTW_INFO("===> %s Rx Error Flag INT Status\n", __func__);
	if (pHalData->IntArray[1]  & IMR_TXFOVW_8188GTV)
		RTW_INFO("===> %s Transmit FIFO Overflow\n", __func__);
	if (pHalData->IntArray[1]  & IMR_RXFOVW_8188GTV)
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	pkt_cnt = GET_RX_STATUS_DESC_USB_AGG_PKTNUM_8188GTV(pbuf);
#endif
	...
	do {
		...
		rtl8188gtv_query_rx_desc_status(precvframe, pbuf);
...
void rtl8188gtvu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8188gtvu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8188GTVU;
	RTW_INFO("CHIP TYPE: RTL8188GTVU\n");
}
