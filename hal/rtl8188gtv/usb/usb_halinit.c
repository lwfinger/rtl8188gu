...
static BOOLEAN HalUsbSetQueuePipeMapping8188GTVUsb(
		PADAPTER	pAdapter,
		u8		NumInPipe,
		u8		NumOutPipe
)
{
	...
	_ConfigChipOutEP_8188GTV(pAdapter, NumOutPipe);
...
void rtl8188gtvu_interface_configure(_adapter *padapter)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(padapter);
	...
	HalUsbSetQueuePipeMapping8188GTVUsb(padapter,
			  pdvobjpriv->RtNumInPipes, pdvobjpriv->RtNumOutPipes);

}

static u32 _InitPowerOn_8188GTVU(PADAPTER padapter)
{
	...
	if ((regsty->pll_ref_clk_sel & 0x0F) != 0x0F)
		rtl8188gtv_set_pll_ref_clk_sel(padapter, regsty->pll_ref_clk_sel);

	/* HW Power on sequence */
	if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8188GTV_card_enable_flow))
		return _FAIL;

	/* Enable MAC DMA/WMAC/SCHEDULE/SEC block */
	/* Set CR bit10 to enable 32k calibration. Suggested by SD1 Gimmy. Added by tynli. 2011.08.31. */
	rtw_write8(padapter, REG_CR_8188GTV, 0x00);  /*suggseted by zhouzhou, by page, 20111230 */
	value16 = rtw_read16(padapter, REG_CR_8188GTV);
	value16 |= (HCI_TXDMA_EN | HCI_RXDMA_EN | TXDMA_EN | RXDMA_EN
		    | PROTOCOL_EN | SCHEDULE_EN | ENSEC | CALTMR_EN);
	rtw_write16(padapter, REG_CR_8188GTV, value16);
...
static void _InitInterrupt(PADAPTER padapter)
{
	...
	rtw_write32(padapter, REG_HISR0_8188GTV, 0xFFFFFFFF);
	rtw_write32(padapter, REG_HISR1_8188GTV, 0xFFFFFFFF);
...
static void _InitQueueReservedPage(PADAPTER padapter)
{
	...
	if (pHalData->OutEpQueueSel & TX_SELE_HQ)
		numHQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_HPQ_8188GTV : NORMAL_PAGE_NUM_HPQ_8188GTV;

	if (pHalData->OutEpQueueSel & TX_SELE_LQ)
		numLQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_LPQ_8188GTV : NORMAL_PAGE_NUM_LPQ_8188GTV;

	/* NOTE: This step shall be proceed before writing REG_RQPN. */
	if (pHalData->OutEpQueueSel & TX_SELE_NQ)
		numNQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_NPQ_8188GTV : NORMAL_PAGE_NUM_NPQ_8188GTV;
	value8 = (u8)_NPQ(numNQ);
	rtw_write8(padapter, REG_RQPN_NPQ, value8);

	numPubQ = TX_TOTAL_PAGE_NUMBER_8188GTV - numHQ - numLQ - numNQ;
...
static void _InitTxBufferBoundary(PADAPTER padapter)
{
	...
	if (!pregistrypriv->wifi_spec)
		txpktbuf_bndy = TX_PAGE_BOUNDARY_8188GTV;
	else {
		/*for WMM */
		txpktbuf_bndy = WMM_NORMAL_TX_PAGE_BOUNDARY_8188GTV;
	}

	rtw_write8(padapter, REG_TXPKTBUF_BCNQ_BDNY_8188GTV, txpktbuf_bndy);
	rtw_write8(padapter, REG_TXPKTBUF_MGQ_BDNY_8188GTV, txpktbuf_bndy);
	rtw_write8(padapter, REG_TXPKTBUF_WMAC_LBK_BF_HD_8188GTV, txpktbuf_bndy);
	...
	rtw_write8(padapter, REG_DWBCN1_CTRL_8188GTV + 1, val8); /* BCN1_HEAD */
...
void
_InitTransferPageSize_8188gtvu(
	PADAPTER Adapter
)
{
	...
	rtw_write8(Adapter, REG_PBP_8188GTV, value8);
}
...
{
	u16 value16		= (rtw_read16(Adapter, REG_TRXDMA_CTRL_8188GTV) & 0x7);

	value16 |=	_TXDMA_BEQ_MAP(beQ)	| _TXDMA_BKQ_MAP(bkQ) |
			_TXDMA_VIQ_MAP(viQ)	| _TXDMA_VOQ_MAP(voQ) |
			_TXDMA_MGQ_MAP(mgtQ) | _TXDMA_HIQ_MAP(hiQ);

	rtw_write16(Adapter, REG_TRXDMA_CTRL_8188GTV, value16);
}
...
{
	/* RX Page Boundary */
	u16 rxff_bndy = RX_DMA_BOUNDARY_8188GTV;
...
)
{
	...
	value16 = 0xFFFF;
	rtw_write16(Adapter, REG_RXFLTMAP2_8188GTV, value16);
	...
	value16 = 0x400;
	rtw_write16(Adapter, REG_RXFLTMAP1_8188GTV, value16);

	/* Accept all management frames */
	value16 = 0xFFFF;
	rtw_write16(Adapter, REG_RXFLTMAP0_8188GTV, value16);
...
static void
_InitRDGSetting_8188gtvu(
		PADAPTER Adapter
)
{
	rtw_write8(Adapter, REG_RD_CTRL_8188GTV, 0xFF);
	rtw_write16(Adapter, REG_RD_NAV_NXT_8188GTV, 0x200);
	rtw_write8(Adapter, REG_RD_RESP_PKT_TH_8188GTV, 0x05);
}
...
static void _InitBurstPktLen(PADAPTER padapter)
{
	...
	tmp8 = rtw_read8(padapter, REG_RXDMA_PRO_8188GTV);
	tmp8 &= ~(BIT(4) | BIT(5));
	...
	rtw_write8(padapter, REG_RXDMA_PRO_8188GTV, tmp8);

	pHalData->bSupportUSB3 = _FALSE;

	tmp8 = rtw_read8(padapter, REG_HT_SINGLE_AMPDU_8188GTV);
	tmp8 |= BIT(7); /* enable single pkt ampdu */
	rtw_write8(padapter, REG_HT_SINGLE_AMPDU_8188GTV, tmp8);
	rtw_write16(padapter, REG_MAX_AGGR_NUM_8188GTV, 0x0C14);
	rtw_write8(padapter, REG_AMPDU_MAX_TIME_8188GTV, 0x70);
	rtw_write32(padapter, REG_AMPDU_MAX_LENGTH_8188GTV, 0xffffffff);
	if (pHalData->AMPDUBurstMode)
		rtw_write8(padapter, REG_AMPDU_BURST_MODE_8188GTV, 0x5F);

	/* for VHT packet length 11K */
	rtw_write8(padapter, REG_RX_PKT_LIMIT_8188GTV, 0x18);

	rtw_write8(padapter, REG_PIFS_8188GTV, 0x00);
	rtw_write8(padapter, REG_FWHW_TXQ_CTRL_8188GTV, 0x80);
	rtw_write32(padapter, REG_FAST_EDCA_CTRL_8188GTV, 0x03086666);
	rtw_write8(padapter, REG_USTIME_TSF_8188GTV, 0x28);
	rtw_write8(padapter, REG_USTIME_EDCA_8188GTV, 0x28);

	/* to prevent mac is reseted by bus. 20111208, by Page */
	tmp8 = rtw_read8(padapter, REG_RSV_CTRL_8188GTV);
	tmp8 |= BIT(5) | BIT(6);
	rtw_write8(padapter, REG_RSV_CTRL_8188GTV, tmp8);
}
...
)
{
	...
	if (pHalData->UsbTxAggMode) {
		value32 = rtw_read32(Adapter, REG_DWBCN0_CTRL_8188GTV);
		value32 = value32 & ~(BLK_DESC_NUM_MASK << BLK_DESC_NUM_SHIFT);
		value32 |= ((pHalData->UsbTxAggDescNum & BLK_DESC_NUM_MASK) << BLK_DESC_NUM_SHIFT);

		rtw_write32(Adapter, REG_DWBCN0_CTRL_8188GTV, value32);
		rtw_write8(Adapter, REG_DWBCN1_CTRL_8188GTV, pHalData->UsbTxAggDescNum << 1);
	}
...
static void
PHY_InitAntennaSelection8188GTV(
	PADAPTER Adapter
)
{
...
)
{
	...
	EFUSE_ShadowRead(Adapter, 1, EEPROM_FEATURE_OPTION_8188GTV, (u32 *)&tmpvalue);
...
#ifdef CONFIG_8188GTVTV_SOLUTION_D
/* 
...
void rtl8188gtvu_solution_d(PADAPTER Adapter)
{
	u8 reg_val[6] = {0};
...
u32 rtl8188gtvu_hal_init(PADAPTER padapter)
{
...
#undef REG_USB_ACCESS_TIMEOUT
	...
	value8 = rtw_read8(padapter, REG_SYS_CLKR_8188GTV + 1);
	u1bRegCR = rtw_read8(padapter, REG_CR_8188GTV);
	...
	if (!pregistrypriv->wifi_spec)
		boundary = TX_PAGE_BOUNDARY_8188GTV;
	else {
		/* for WMM */
		boundary = WMM_NORMAL_TX_PAGE_BOUNDARY_8188GTV;
	}
	status =  rtl8188gtv_InitLLTTable(padapter);
	if (status == _FAIL) {
		goto exit;
	}
	...
	if (pHalData->bRDGEnable)
		_InitRDGSetting_8188gtvu(padapter);
	...
	if (pHalData->AMPDUBurstMode) {
		...
		value8 = rtw_read8(padapter, REG_EARLY_MODE_CONTROL_8188GTV);
#if RTL8188GTV_EARLY_MODE_PKT_NUM_10 == 1
		value8 = value8 | 0x1f;
#else
		value8 = value8 | 0xf;
#endif
		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8188GTV, value8);

		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8188GTV + 3, 0x80);

		value8 = rtw_read8(padapter, REG_TCR_8188GTV + 1);
		value8 = value8 | 0x40;
		rtw_write8(padapter, REG_TCR_8188GTV + 1, value8);
	} else
		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8188GTV, 0);
#endif
	...
	if (padapter->registrypriv.mp_mode == 0
		...
		status = rtl8188gtv_FirmwareDownload(padapter, _FALSE);
		if (status != _SUCCESS) {
	...
	PHY_InitAntennaSelection8188GTV(padapter);



	HAL_INIT_PROFILE_TAG(HAL_INIT_STAGES_MAC);
#if (HAL_MAC_ENABLE == 1)
	status = PHY_MACConfig8188GTV(padapter);
	if (status == _FAIL) {
		RTW_INFO("PHY_MACConfig8188GTV fault !!\n");
		goto exit;
	}
#endif
	RTW_INFO("PHY_MACConfig8188GTV OK!\n");
	...
	/* */
#if (HAL_BB_ENABLE == 1)
	status = PHY_BBConfig8188GTV(padapter);
	if (status == _FAIL) {
		RTW_INFO("PHY_BBConfig8188GTV fault !!\n");
		goto exit;
	}
#endif

	RTW_INFO("PHY_BBConfig8188GTV OK!\n");



	HAL_INIT_PROFILE_TAG(HAL_INIT_STAGES_RF);
#if (HAL_RF_ENABLE == 1)
	status = PHY_RFConfig8188GTV(padapter);

	if (status == _FAIL) {
		RTW_INFO("PHY_RFConfig8188GTV fault !!\n");
		goto exit;
	}

	RTW_INFO("PHY_RFConfig8188GTV OK!\n");
	...
	_InitTransferPageSize_8188gtvu(padapter);
	...
	rtl8188gtv_InitBeaconMaxError(padapter, _TRUE);
	...
	BBTurnOnBlock_8188GTV(padapter);
	...
	rtl8188gtv_InitAntenna_Selection(padapter);
...
#endif
	{
		...
		if (pwrctrlpriv->rf_pwrstate == rf_on) {
			...
			/*phy_lc_calibrate_8188gtv(&pHalData->odmpriv);*/
			halrf_lck_trigger(&pHalData->odmpriv);

#ifdef CONFIG_BT_COEXIST
			/* Inform WiFi FW that it is the beginning of IQK */
			h2cCmdBuf = 1;
			FillH2CCmd8188GTV(padapter, H2C_8188GTV_BT_WLAN_CALIBRATION, 1, &h2cCmdBuf);
	...
	/* #if RTL8188GTV_USB_MAC_LOOPBACK */
#if 0
	rtw_write8(padapter, REG_CR_8188GTV + 3, 0x0B);
	RTW_INFO("MAC loopback: REG_CR_8188GTV=%#X.\n", rtw_read32(padapter, REG_CR_8188GTV));
#endif

#if FPGA_TWO_MAC_VERIFICATION
	/* #if 1 */
	/* Enable BB */
	value8 = rtw_read8(padapter, REG_SYS_FUNC_EN_8188GTV);
	RTW_INFO("RJZ: open 2-MAC mode: REG_SYS_FUNC_EN_8188GTV = %#x.\n", value8);
	rtw_write8(padapter, REG_SYS_FUNC_EN_8188GTV, value8 | BIT0 | BIT1 | BIT2);
	value8 = rtw_read8(padapter, REG_SYS_FUNC_EN_8188GTV);
	RTW_INFO("RJZ: open 2-MAC mode: REG_SYS_FUNC_EN_8188GTV = %#x.\n", value8);

	/* Use 40MHz */
	value8 = rtw_read8(padapter, REG_SYS_CLKR_8188GTV);
	rtw_write8(padapter, REG_SYS_CLKR_8188GTV, value8 & ~BIT4);
	value8 = rtw_read8(padapter, REG_SYS_CLKR_8188GTV);
...
static void rtl8188gtvu_hw_power_down(_adapter *padapter)
{
	u8	u1bTmp;

	RTW_INFO("PowerDownRTL8188GTVU\n");
...
/* */
/* Description: RTL8188GTV card disable power sequence v003 which suggested by Scott. */
/* First created by tynli. 2011.01.28. */
/* */
void
CardDisableRTL8188GTVU(
	PADAPTER			Adapter
)
{
	u8		u1bTmp;
	...
	RTW_INFO("CardDisableRTL8188GTVU\n");
	...
	/* stop rx */
	rtw_write8(Adapter, REG_CR_8188GTV, 0x0);
	if ((rtw_read8(Adapter, REG_MCUFWDL_8188GTV) & BIT7) &&
	    GET_HAL_DATA(Adapter)->bFWReady)   /*8051 RAM code */
		rtl8188gtv_FirmwareSelfReset(Adapter);

	/* 1. Run LPS WL RFOFF flow */
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8188GTV_enter_lps_flow);

	/* Reset MCU. Suggested by Filen. 2011.01.26. by tynli. */
	u1bTmp = rtw_read8(Adapter, REG_SYS_FUNC_EN_8188GTV + 1);
	rtw_write8(Adapter, REG_SYS_FUNC_EN_8188GTV + 1, (u1bTmp & (~BIT2)));

	/* MCUFWDL 0x80[1:0]=0				// reset MCU ready status */
	rtw_write8(Adapter, REG_MCUFWDL_8188GTV, 0x00);

	/* Card disable power action flow */
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8188GTV_card_disable_flow);

	GET_HAL_DATA(Adapter)->bFWReady = _FALSE;
}

u32 rtl8188gtvu_hal_deinit(PADAPTER Adapter)
{
	...
	rtw_write32(Adapter, REG_HISR0_8188GTV, 0xFFFFFFFF);
	rtw_write32(Adapter, REG_HISR1_8188GTV, 0xFFFFFFFF);

#if 0
	/* USB only need to clear HISR, no need to set HIMR, because there's no hardware interrupt for USB. */
	rtw_write32(Adapter, REG_HIMR0_8188GTV, IMR_DISABLED_8188GTV);
	rtw_write32(Adapter, REG_HIMR1_8188GTV, IMR_DISABLED_8188GTV);
#endif
	...
	if (pwrctl->bkeepfwalive) {
		...
		if ((pwrctl->bHWPwrPindetect) && (pwrctl->bHWPowerdown))
			rtl8188gtvu_hw_power_down(Adapter);
	} else
#endif
	{
...
unsigned int rtl8188gtvu_inirp_init(PADAPTER Adapter)
{
	u8 i;
...
unsigned int rtl8188gtvu_inirp_deinit(PADAPTER Adapter)
{
#ifdef CONFIG_USB_INTERRUPT_IN_PIPE
	u32(*_read_interrupt)(struct intf_hdl *pintfhdl, u32 addr);
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
)
{
	...
	if (AutoLoadFail) {
		...
		/* VID, PID */
		pHalData->EEPROMVID = le16_to_cpu(*(u16 *)&hwinfo[EEPROM_VID_8188GTVU]);
		pHalData->EEPROMPID = le16_to_cpu(*(u16 *)&hwinfo[EEPROM_PID_8188GTVU]);
...
static u8
InitAdapterVariablesByPROM_8188GTVU(
		PADAPTER	padapter
)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	...
	if (sizeof(pHalData->efuse_eeprom_data) < HWSET_MAX_SIZE_8188GTV)
		RTW_INFO("[WARNING] size of efuse_eeprom_data is less than HWSET_MAX_SIZE_8188GTV!\n");
	...
	Hal_EfuseParsePIDVID_8188GTVU(padapter, hwinfo, pHalData->bautoload_fail_flag);
	Hal_EfuseParseEEPROMVer_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	hal_config_macaddr(padapter, pHalData->bautoload_fail_flag);
	Hal_EfuseParseTxPowerInfo_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	/* Hal_EfuseParseBTCoexistInfo_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag); */

	Hal_EfuseParseChnlPlan_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	Hal_EfuseParseThermalMeter_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	/* _ReadLEDSetting(Adapter, PROMContent, pHalData->bautoload_fail_flag); */
	Hal_EfuseParsePowerSavingMode_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	Hal_EfuseParseAntennaDiversity_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);

	Hal_EfuseParseEEPROMVer_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	Hal_EfuseParseCustomerID_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	/* Hal_EfuseParseRateIndicationOption(padapter, hwinfo, pHalData->bautoload_fail_flag); */
	Hal_EfuseParseXtal_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
	/* */
	...
	/*hal_CustomizedBehavior_8188GTVU(Adapter); */

	Hal_EfuseParseKFreeData_8188GTV(padapter, hwinfo, pHalData->bautoload_fail_flag);
...
#ifdef CONFIG_8188GTVTV_SOLUTION_D
	rtl8188gtvu_solution_d(Adapter);
#endif

	if (InitAdapterVariablesByPROM_8188GTVU(Adapter) != _SUCCESS)
		goto exit;
...
static u8 ReadAdapterInfo8188GTVU(PADAPTER Adapter)
{
	u8 ret = _FAIL;

	/* Read EEPROM size before call any EEPROM function */
	Adapter->EepromAddressSize = GetEEPROMSize8188GTV(Adapter);
...
static void rtl8188gtvu_trigger_gpio_0(_adapter *padapter)
{
 ...
 * some variables will be processed in SetHwReg8188GTVU()
 */
u8 SetHwReg8188GTVU(PADAPTER Adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
			...
			if (threshold == 0) {
				switch (pHalData->rxagg_mode) {
					case RX_AGG_DMA:
						...
						if (agg_size > RX_DMA_BOUNDARY_8188GTV)
							agg_size = RX_DMA_BOUNDARY_8188GTV >> 1;
	...
	case HW_VAR_TRIGGER_GPIO_0:
		rtl8188gtvu_trigger_gpio_0(Adapter);
		break;

	default:
		ret = SetHwReg8188GTV(Adapter, variable, val);
		break;
	}
...
/*
 * If variable not handled here,
 * some variables will be processed in GetHwReg8188GTVU()
 */
void GetHwReg8188GTVU(PADAPTER Adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	...
	case HW_VAR_CPWM:
		...
		*val = rtw_read8(Adapter, REG_USB_HCPWM);
		...
		break;
	default:
		GetHwReg8188GTV(Adapter, variable, val);
		break;
	}
...
)
{
	...
	case HW_VAR_MAX_RX_AMPDU_FACTOR:
		*((HT_CAP_AMPDU_FACTOR *)pValue) = MAX_AMPDU_FACTOR_64K;
		break;
	default:
		bResult = GetHalDefVar8188GTV(Adapter, eVariable, pValue);
		break;
	}
...
)
{
	...
	switch (eVariable) {
	default:
		bResult = SetHalDefVar8188GTV(Adapter, eVariable, pValue);
		break;
	}
...
static u8 rtl8188gtvu_ps_func(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id, u8 *val)
{
	u8 bResult = _TRUE;
	switch (efunc_id) {
	...
	case HAL_USB_SELECT_SUSPEND: {
		...
		rtl8188gtv_set_FwSelectSuspend_cmd(Adapter, bfwpoll , 500); /*note fw to support hw power down ping detect */
	}
	break;
...
void rtl8188gtvu_set_hal_ops(_adapter *padapter)
{
	...
	rtl8188gtv_set_hal_ops(pHalFunc);

	pHalFunc->hal_power_on = &_InitPowerOn_8188GTVU;
	pHalFunc->hal_power_off = &CardDisableRTL8188GTVU;

	pHalFunc->hal_init = &rtl8188gtvu_hal_init;
	pHalFunc->hal_deinit = &rtl8188gtvu_hal_deinit;

	pHalFunc->inirp_init = &rtl8188gtvu_inirp_init;
	pHalFunc->inirp_deinit = &rtl8188gtvu_inirp_deinit;

	pHalFunc->init_xmit_priv = &rtl8188gtvu_init_xmit_priv;
	pHalFunc->free_xmit_priv = &rtl8188gtvu_free_xmit_priv;

	pHalFunc->init_recv_priv = &rtl8188gtvu_init_recv_priv;
	pHalFunc->free_recv_priv = &rtl8188gtvu_free_recv_priv;
#ifdef CONFIG_RTW_SW_LED
	pHalFunc->InitSwLeds = &rtl8188gtvu_InitSwLeds;
	pHalFunc->DeInitSwLeds = &rtl8188gtvu_DeInitSwLeds;
#endif/*CONFIG_RTW_SW_LED */

	pHalFunc->init_default_value = &rtl8188gtv_init_default_value;
	pHalFunc->intf_chip_configure = &rtl8188gtvu_interface_configure;
	pHalFunc->read_adapter_info = &ReadAdapterInfo8188GTVU;

	pHalFunc->set_hw_reg_handler = &SetHwReg8188GTVU;
	pHalFunc->GetHwRegHandler = &GetHwReg8188GTVU;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8188GTVUsb;
	pHalFunc->SetHalDefVarHandler = &SetHalDefVar8188GTVUsb;

	pHalFunc->hal_xmit = &rtl8188gtvu_hal_xmit;
	pHalFunc->mgnt_xmit = &rtl8188gtvu_mgnt_xmit;
	pHalFunc->hal_xmitframe_enqueue = &rtl8188gtvu_hal_xmitframe_enqueue;

#ifdef CONFIG_HOSTAPD_MLME
	pHalFunc->hostap_mgnt_xmit_entry = &rtl8188gtvu_hostap_mgnt_xmit_entry;
#endif
	pHalFunc->interface_ps_func = &rtl8188gtvu_ps_func;

#ifdef CONFIG_XMIT_THREAD_MODE
	pHalFunc->xmit_thread_handler = &rtl8188gtvu_xmit_buf_handler;
#endif
#ifdef CONFIG_SUPPORT_USB_INT
	pHalFunc->interrupt_handler = interrupt_handler_8188gtvu;
#endif
